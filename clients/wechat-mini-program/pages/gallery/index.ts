...
Page({
  data: { assets: [] as any[], selectedCount: 0, targetCount: 0, confirmed: false, syncing: false },
  onShow() { this.refresh(); selectionStore.syncPending().finally(() => this.refresh()); },
  refresh() { this.setData({ assets: selectionStore.assets.map(asset => ({ ...asset, selected: selectionStore.isSelected(asset.selectionAssetId), favorite: selectionStore.isFavorite(asset.selectionAssetId) })), selectedCount: selectionStore.selectedCount(), targetCount: selectionStore.project?.targetCount ?? 0, confirmed: selectionStore.confirmed, syncing: selectionStore.pendingRequests.length > 0 }); },
