...
| WPF process-per-fixture, x64 | DONE on passing full run: 97 fixtures, 1199/1199, 0 failed, 0 skipped; see intermittent-test note below | `artifacts/rc12-wpf-process-isolation/eagle-shell-final-head-rerun/rc12-wpf-process-isolation.json` |
...
At report-checkpoint HEAD `806605a`, the first complete x64 isolation run reported 1198 passed / 1 failed / 0 skipped: `EmbeddedAssetLibraryWpfTests.RealSplitterDragKeepsSideWidthBindingsAndCollapseRestoresTheDraggedWidths` timed out waiting for a synthetic `Key.Right` event on the thumbnail-size slider to update workspace settings. The same test alone passed 1/1; its complete isolated fixture passed 25/25; a subsequent complete run at the same HEAD passed 1199/1199. Raw failed-run and passing-run manifests are retained at `artifacts/rc12-wpf-process-isolation/eagle-shell-final-head/` and `eagle-shell-final-head-rerun/`. This is an intermittent test/keyboard-event timing risk, not proven resolved merely by rerunning; physical keyboard operation remains PARTIAL. No test was skipped or assertion relaxed.
