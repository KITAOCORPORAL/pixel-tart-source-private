...
public sealed class OrganizePhotosViewModel : ObservableObject
{
    ...
    public OrganizePhotosViewModel(OrganizeService service, IDialogService dialogs, TaskOperationBridge? taskOperationBridge = null)
    {
        _service = service;
        _dialogs = dialogs;
    ...
    public async Task AddPathsAsync(IEnumerable<string> paths)
    {
        ...
        IsBusy = true;
        StatusMessage = "正在读取照片与元数据…";
        _cancellation = new();
        try
        {
            await RunTrackedAsync("整理图片扫描", string.Empty, async (context, token) =>
            {
                var progress = new Progress<OrganizeExecutionProgress>(x =>
                {
                    Progress = x.Total == 0 ? 0 : x.Completed * 100d / x.Total;
                    if (context is not null)
                    {
                        var summary = new TaskResultSummary(x.Total, x.Completed, 0, 0, 0, 0, 0, 0);
                        _ = context.ReportProgressAsync(Progress, "读取照片与元数据", x.CurrentFile, summary, token);
                    }
                });
                var scanned = await _service.ScanAsync(values, token, progress);
                foreach (var photo in scanned)
                    if (Photos.All(x => !string.Equals(x.SourcePath, photo.SourcePath, StringComparison.OrdinalIgnoreCase))) Photos.Add(photo);
                Regroup();
                StatusMessage = $"已读取 {Photos.Count} 个文件";
                return new TaskResultSummary(scanned.Count, scanned.Count, 0, 0, 0, 0, scanned.Sum(x => x.FileSizeBytes), 0);
            }, _cancellation.Token);
        }
    ...
    private async Task ExecuteAsync()
    {
        ...
        if (CurrentPlan.OperationType == OrganizeOperationType.SavePlan)
        {
            ...
            StatusMessage = $"整理方案已保存：{await _service.SavePlanAsync(CurrentPlan, planPath)}";
            return;
        }
        if (CurrentPlan.OperationType == OrganizeOperationType.Move && !_dialogs.Confirm($"将移动 {CurrentPlan.Items.Count} 个文件。目标校验成功后才删除源文件，是否继续？", "移动文件二次确认")) return;
        ...
        try
        {
            ...
            await RunTrackedAsync("整理图片" + (CurrentPlan.OperationType == OrganizeOperationType.Move ? "移动" : "复制"), resourceSnapshot, async (context, token) =>
            {
                var progress = new Progress<OrganizeExecutionProgress>(x =>
                {
                    Progress = x.Total == 0 ? 0 : x.Completed * 100d / x.Total;
                    StatusMessage = string.IsNullOrWhiteSpace(x.CurrentFile) ? "整理完成" : $"正在处理 {Path.GetFileName(x.CurrentFile)}";
                    if (context is not null)
                    {
                        var summary = new TaskResultSummary(x.Total, x.Completed, 0, 0, 0, 0, 0, 0);
                ...
                await _service.ExportReportsAsync(LastResult.Manifest, CurrentPlan.OutputRoot);
                StatusMessage = $"完成 {LastResult.Succeeded}，失败 {LastResult.Failed}，跳过 {LastResult.Skipped}";
                return new TaskResultSummary(LastResult.Manifest.Items.Count, LastResult.Succeeded, LastResult.Failed, LastResult.Skipped, 0, 0, LastResult.Manifest.Items.Sum(x => x.ExpectedSourceSize), LastResult.Manifest.Items.Where(x => x.State is OrganizeItemState.Copied or OrganizeItemState.Moved).Sum(x => x.ExpectedSourceSize));
            }, _cancellation.Token);
        }
        finally
        {
    ...
    private async Task ExportReportsAsync()
    {
        ...
        var folder = _dialogs.ChooseFolder("选择报告输出目录", OutputPath);
        if (folder is null) return;
        await RunTrackedAsync("整理报告生成", $"write-root:{folder}", async (context, token) =>
        {
            await _service.ExportReportsAsync(LastResult.Manifest, folder, token);
            StatusMessage = "CSV、JSON 和 TXT 报告已导出";
            return new TaskResultSummary(3, 3, 0, 0, 0, 0, 0, 0);
    ...
    private async Task UndoMoveAsync()
    {
        ...
        try
        {
            var sourceRoots = LastResult.Manifest.Items.Select(x => Path.GetDirectoryName(x.SourcePath)).Where(x => !string.IsNullOrWhiteSpace(x)).Distinct(StringComparer.OrdinalIgnoreCase).Select(x => $"write-root:{x}");
            var destinationRoots = LastResult.Manifest.Items.Select(x => Path.GetDirectoryName(x.DestinationPath)).Where(x => !string.IsNullOrWhiteSpace(x)).Distinct(StringComparer.OrdinalIgnoreCase).Select(x => $"write-source:{x}");
            await RunTrackedAsync("撤销整理移动", string.Join(Environment.NewLine, sourceRoots.Concat(destinationRoots)), async (context, token) =>
            {
                var success = await _service.UndoMoveAsync(LastResult.Manifest, token);
                StatusMessage = success ? "移动整理已安全撤销" : "撤销前置条件不满足，未执行任何强制覆盖";
                return new TaskResultSummary(LastResult.Manifest.Items.Count, success ? LastResult.Manifest.Items.Count : 0, 0, 0, 0, success ? 0 : LastResult.Manifest.Items.Count, 0, 0);
            }, CancellationToken.None);
        }
        finally { IsBusy = false; }
    }

    private async Task RunTrackedAsync(string name, string inputSnapshot, Func<TaskExecutionContext?, CancellationToken, Task<TaskResultSummary>> operation, CancellationToken cancellationToken)
    {
        if (_taskOperationBridge is null)
        {
            await operation(null, cancellationToken);
            return;
        }
        await _taskOperationBridge.RunAsync(name, (context, token) => operation(context, token), inputSnapshot: inputSnapshot, cancellationToken: cancellationToken);
    }
...
public sealed class CollageViewModel : ObservableObject
{
    ...
    public CollageViewModel(CollageExportService exportService, IDialogService dialogs, TaskOperationBridge? taskOperationBridge = null)
    {
        _exportService = exportService; _dialogs = dialogs; _taskOperationBridge = taskOperationBridge; _selectedTemplate = CollageTemplateCatalog.All[0];
    ...
    public RelayCommand SwapLeftCommand { get; }
    public RelayCommand SwapRightCommand { get; }
    ...
    private async Task ExportAsync()
    {
        ...
        var path = _dialogs.ChooseSaveFile("导出拼图", "JPEG|*.jpg|PNG|*.png", ".jpg", $"像素蛋挞拼图_{DateTime.Now:yyyyMMdd_HHmm}.jpg");
        if (path is null) return;
        Project.Export.Format = string.Equals(Path.GetExtension(path), ".png", StringComparison.OrdinalIgnoreCase) ? "PNG" : "JPG";
        if (Project.Export.Format == "JPG") Project.Export.TransparentBackground = false;
        OnPropertyChanged(nameof(EstimatedExportSizeText));
        if (!_dialogs.Confirm($"输出尺寸：{Project.Export.PixelWidth} × {Project.Export.PixelHeight}\n格式：{Project.Export.Format}\n质量：{Project.Export.JpegQuality}\n透明背景：{Project.Export.TransparentBackground}\n预计文件大小：{EstimatedExportSizeText}\n输出：{path}\n\n默认不覆盖，冲突时自动编号。是否导出？", "确认拼图导出")) return;
        IsBusy = true;
        _cancellation = new();
        StatusMessage = "正在从原图导出…";
        try
        {
            CollageExportResult? result = null;
            if (_taskOperationBridge is null)
            {
                result = await _exportService.ExportAsync(Project, path, _cancellation.Token);
            }
            else
            {
                await _taskOperationBridge.RunAsync("拼图导出", async (context, token) =>
                {
                    using var linked = CancellationTokenSource.CreateLinkedTokenSource(token, _cancellation.Token);
                    await context.SafeBoundaryAsync("渲染拼图", 0, cancellationToken: linked.Token);
                    result = await _exportService.ExportAsync(Project, path, linked.Token);
                    var summary = new TaskResultSummary(1, 1, 0, 0, 0, 0, Project.Images.Sum(x => new FileInfo(x.SourcePath).Length), result.FileSizeBytes);
                    await context.ReportProgressAsync(100, "拼图导出完成", result.OutputPath, summary, linked.Token);
                    return summary;
                }, inputSnapshot: $"write-root:{Path.GetDirectoryName(path)}", cancellationToken: _cancellation.Token);
            }
            if (result is null) return;
            StatusMessage = $"已导出 {result.PixelWidth}×{result.PixelHeight}，{result.FileSizeBytes / 1024d / 1024:0.##} MB";
            if (_dialogs.Confirm("拼图已导出，是否在资源管理器中定位文件？", "导出完成")) _dialogs.RevealFile(result.OutputPath);
        }
        catch (OperationCanceledException) { StatusMessage = "已取消导出，不完整文件已清理"; }
        catch (Exception ex) { _dialogs.ShowError(ex.Message); StatusMessage = "导出失败"; }
        finally
        {
            IsBusy = false;
            _cancellation.Dispose();
    ...
    private void RefreshCommandStates() { AddPhotosCommand.RaiseCanExecuteChanged(); ClearCommand.RaiseCanExecuteChanged(); RemoveCommand.RaiseCanExecuteChanged(); ReplaceCommand.RaiseCanExecuteChanged(); SwapLeftCommand.RaiseCanExecuteChanged(); SwapRightCommand.RaiseCanExecuteChanged(); RotateCommand.RaiseCanExecuteChanged(); FlipHorizontalCommand.RaiseCanExecuteChanged(); FlipVerticalCommand.RaiseCanExecuteChanged(); ResetImageCommand.RaiseCanExecuteChanged(); ExportCommand.RaiseCanExecuteChanged(); CancelCommand.RaiseCanExecuteChanged(); }
}
