...
internal sealed class AssetLibraryP2AutomatedAcceptanceController : IAssetLibraryLoadStateController
{
    ...
    internal const string SourceHeadEnvironmentVariable = "PIXEL_TART_P2_AUTOMATED_SOURCE_HEAD";
    internal const string FixtureRootEnvironmentVariable = "PIXEL_TART_P2_AUTOMATED_FIXTURE_ROOT";
    internal const string PlanSchema = "pixel-tart-p2-automated-plan/v1";
    internal const string EventSchema = "pixel-tart-p2-automated-event/v1";
    internal const string SummarySchema = "pixel-tart-p2-automated-summary/v1";
    ...
    internal const string FixtureIntegrityScenario = "fixture-integrity/v1";
    internal const string OrganizationBrowserScenario = "organization-browser/v1";
    internal const string SmartTagQueryScenario = "smart-tag-query/v1";
    internal const string FourViewsQuerySortScenario = "four-views-query-sort/v1";
    internal const string SelectionLargeScenario = "selection-large/v1";
    internal const string MetadataDragCommandScenario = "metadata-drag-command/v1";
    internal const string InspectorStatesScenario = "inspector-states/v1";
    internal const string ResilienceStatesScenario = "resilience-states/v1";
    internal const string RestartPersistenceScenario = "restart-persistence/v1";
    internal const string LayoutDpiPerformanceScenario = "layout-dpi-performance/v1";
    ...
    [
        FixtureIntegrityScenario,
        OrganizationBrowserScenario,
        SmartTagQueryScenario,
        FourViewsQuerySortScenario,
        SelectionLargeScenario,
        MetadataDragCommandScenario,
        InspectorStatesScenario,
        ResilienceStatesScenario,
        RestartPersistenceScenario,
        LayoutDpiPerformanceScenario,
    ];

    private const string QueryFailureInjectionId = "asset-library-p2-automated-initial-query-io-once/v1";
    ...
    private AssetLibraryP2AutomatedAcceptanceController(
        ...
        string? fixtureRoot)
    {
        ...
        _applicationAssemblyPath = NormalizePath(typeof(AssetLibraryP2AutomatedAcceptanceController).Assembly.Location);
    ...
    internal static AssetLibraryP2AutomatedAcceptanceController? TryCreate(string applicationRoot, ILogService? logService)
    {
        ...
        try
        {
            ...
            logService?.Info("P2 Asset Library automated in-process WPF acceptance enabled in the isolated Dev Preview runtime.");
            return new(applicationRoot, plan, runRoot!, planPath!, sourceHead!, fixtureRoot);
        }
        catch (Exception exception)
        {
            logService?.Error("P2 Asset Library automated acceptance opt-in was rejected.", exception);
            throw;
        }
    }
    ...
    internal void Observe(MainViewModel viewModel, AssetLibraryP2AutomatedAcceptanceDriver driver)
    {
        ArgumentNullException.ThrowIfNull(viewModel);
    ...
    {
        if (!string.Equals(_scenarioId, ResilienceStatesScenario, StringComparison.Ordinal))
    ...
    {
        if (!string.Equals(_scenarioId, FixtureIntegrityScenario, StringComparison.Ordinal) || IsRestartPhase)
            throw new InvalidOperationException("Only the fixture-integrity primary scenario may record fixture bootstrap evidence.");
        var import = new
        {
            mechanism,
    ...
    internal ArtifactState WriteArtifact(string scenarioId, string kind, string fileName, ReadOnlySpan<byte> content)
    {
        ...
        var directoryName = kind switch
        {
            ...
            var value when value.Contains("query", StringComparison.OrdinalIgnoreCase) => "queries",
            ...
            var value when value.Contains("command", StringComparison.OrdinalIgnoreCase) => "commands",
            var value when value.Contains("inspector", StringComparison.OrdinalIgnoreCase) => "inspectors",
            var value when value.Contains("performance", StringComparison.OrdinalIgnoreCase) => "performance",
            var value when value.Contains("database", StringComparison.OrdinalIgnoreCase) => "databases",
            _ => "screenshots",
        };
    ...
    public async Task BeforeRepositoryInitializationAsync(int attempt, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(_fixtureRoot))
            throw new InvalidOperationException("Every P2 primary scenario requires the run-owned 512-item fixture.");
        var fixtureDatabase = NormalizePath(Path.Combine(_fixtureRoot, "asset-library-v16.db"));
        EnsurePathInside(fixtureDatabase, _fixtureRoot, "fixture database");
        if (!File.Exists(fixtureDatabase))
            throw new FileNotFoundException("The run-owned P2 fixture database is missing.", fixtureDatabase);
        Directory.CreateDirectory(Path.GetDirectoryName(_databasePath)!);
        if (File.Exists(_databasePath))
            throw new InvalidOperationException("A primary P2 scenario may not reuse an existing runtime database.");
        File.Copy(fixtureDatabase, _databasePath, overwrite: false);
        RecordEvent(_scenarioId, "run-owned-fixture-copied-before-repository-initialization", "SqliteAssetLibraryRepository", null,
            new { attempt, fixture_database = fixtureDatabase, fixture_sha256 = HashFile(fixtureDatabase), total_count = 512, active_count = 500, archived_count = 12 }, null);
        ...
        CancellationToken cancellationToken)
    {
        ...
        cancellationToken.ThrowIfCancellationRequested();
        if (!IsRestartPhase &&
            string.Equals(_scenarioId, ResilienceStatesScenario, StringComparison.Ordinal) &&
            attempt == 1 &&
            Interlocked.CompareExchange(ref _queryFailureInjected, 1, 0) == 0)
        {
            var exception = new IOException("Deterministic P2 automated recoverable initial-query failure.");
    ...
    {
        if (string.Equals(_scenarioId, RestartPersistenceScenario, StringComparison.Ordinal))
        {
    ...
    private static async Task ValidateDatabaseEvidenceAsync(SqliteConnection connection, ScenarioState state)
    {
        ...
        if (assetCount != 512 || activeCount != 500 || archivedCount != 12)
            throw new InvalidDataException($"The P2 fixture contract differs (total={assetCount}, active={activeCount}, archived={archivedCount}).");
    ...
    {
        if (_scenarioId is not RestartPersistenceScenario)
            throw new InvalidOperationException("Only restart-persistence/v1 may have a restart phase.");
        if (!File.Exists(_databasePath))
        ...
        string? sourceHead)
    {
        ...
        if (!string.Equals(processName, ExpectedProcessName, StringComparison.Ordinal))
            throw new InvalidOperationException("Automated P2 acceptance is restricted to the Modular Harness Dev Preview executable.");

        foreach (var binaryPath in new[]
                 {
                     Environment.ProcessPath!,
                     typeof(AssetLibraryP2AutomatedAcceptanceController).Assembly.Location,
    ...
    private static AutomatedPlan ReadPlan(string planPath)
    {
        ...
        if (phase == "primary" && (ids.Length != 1 || !FixedScenarioIds.Contains(ids[0], StringComparer.Ordinal)))
            throw new InvalidOperationException("A primary automated process must contain exactly one fixed P2 scenario.");
        if (phase == "restart" &&
            (ids.Length != 1 || ids[0] is not RestartPersistenceScenario))
            throw new InvalidOperationException("A restart automated process must contain restart-persistence/v1 only.");
    ...
    private ScenarioState EnsureScenario(string scenarioId) =>
        _scenarios.TryGetValue(scenarioId, out var state)
            ? state
            : throw new InvalidOperationException($"Unknown automated P2 scenario '{scenarioId}'.");
    ...
    private void ValidateLiveBinaryIdentityAtExit()
    {
        ...
        var applicationPath = NormalizePath(typeof(AssetLibraryP2AutomatedAcceptanceController).Assembly.Location);
        ...
        [property: JsonPropertyName("asset_module_sha256")] string AssetModuleSha256)
    {
        ...
        public string HistoricalManualGate => AssetLibraryP2AutomatedAcceptanceController.HistoricalManualGate;
    ...
    private sealed class DatabaseState
    {
        ...
        internal string SchemaQuery { get; set; } = AssetLibraryP2AutomatedAcceptanceController.SchemaQuery;
        ...
        internal void Load(JsonElement database)
        {
            ...
            SchemaVersion = database.TryGetProperty("schema_version", out var version) && version.TryGetInt32(out var schemaVersion)
                ? schemaVersion
                : null;
            SchemaQuery = GetString(database, "schema_query") ?? AssetLibraryP2AutomatedAcceptanceController.SchemaQuery;
