...
internal sealed class AssetLibraryP3AutomatedAcceptanceController : IAssetLibraryLoadStateController
{
    internal const string OptInEnvironmentVariable = "PIXEL_TART_P3_AUTOMATED_ACCEPTANCE";
    internal const string RunRootEnvironmentVariable = "PIXEL_TART_P3_AUTOMATED_RUN_ROOT";
    internal const string PlanPathEnvironmentVariable = "PIXEL_TART_P3_AUTOMATED_PLAN_PATH";
    internal const string SourceHeadEnvironmentVariable = "PIXEL_TART_P3_AUTOMATED_SOURCE_HEAD";
    internal const string FixtureRootEnvironmentVariable = "PIXEL_TART_P3_AUTOMATED_FIXTURE_ROOT";
    ...
    internal const string ImeCancellationScenario = "ime-cancellation/v1";
    internal const string SearchSuggestionsHistoryScenario = "search-suggestions-history/v1";
    internal const string FolderAnyAllNotScenario = "folder-any-all-not/v1";
    internal const string TagAnyAllNotScenario = "tag-any-all-not/v1";
    internal const string ScalarNullCompositionScenario = "scalar-null-composition/v1";
    internal const string VisualCompositionScenario = "visual-composition/v1";
    internal const string NestedCanonicalQueryScenario = "nested-canonical-query/v1";
    internal const string InvalidQueryFailClosedScenario = "invalid-query-fail-closed/v1";
    internal const string SmartFolderLifecyclePreviewScenario = "smart-folder-lifecycle-preview/v1";
    internal const string SmartFolderInvalidMigrationScenario = "smart-folder-invalid-migration/v1";
    internal const string TagManagerLifecycleScenario = "tag-manager-lifecycle/v1";
    internal const string BulkMetadataJournalScenario = "bulk-metadata-journal/v1";
    internal const string FourViewResilienceLayoutScenario = "four-view-resilience-layout/v1";
    ...
    [
        ScopeSwitchScenario,
        ImeCancellationScenario,
        SearchSuggestionsHistoryScenario,
        FolderAnyAllNotScenario,
        TagAnyAllNotScenario,
        ScalarNullCompositionScenario,
        VisualCompositionScenario,
        NestedCanonicalQueryScenario,
        InvalidQueryFailClosedScenario,
        SmartFolderLifecyclePreviewScenario,
        SmartFolderInvalidMigrationScenario,
        TagManagerLifecycleScenario,
        BulkMetadataJournalScenario,
        FourViewResilienceLayoutScenario,
    ];

    internal static readonly string[] RestartScenarioIds =
    [
        SearchSuggestionsHistoryScenario,
        SmartFolderLifecyclePreviewScenario,
        BulkMetadataJournalScenario,
    ...
    private AssetLibraryP3AutomatedAcceptanceController(
        ...
        string? fixtureRoot)
    {
        ...
        _fixtureVariant = plan.FixtureVariant;
        _fixtureDatabasePath = NormalizePath(plan.FixtureDatabasePath);
        ...
        _applicationAssemblyPath = NormalizePath(typeof(AssetLibraryP3AutomatedAcceptanceController).Assembly.Location);
        ...
        if (string.IsNullOrWhiteSpace(_fixtureRoot))
            throw new InvalidOperationException("Every P3 automated process requires the run-owned fixture root.");
        EnsurePathInside(_fixtureDatabasePath, _fixtureRoot, "fixture database path");
        if (!File.Exists(_fixtureDatabasePath))
            throw new FileNotFoundException("The selected P3 fixture database is missing.", _fixtureDatabasePath);
    ...
    internal string FixtureVariant => _fixtureVariant;
    internal string FixtureDatabasePath => _fixtureDatabasePath;
    ...
    internal static AssetLibraryP3AutomatedAcceptanceController? TryCreate(string applicationRoot, ILogService? logService)
    {
        ...
        try
        {
            ...
            logService?.Info("P3 Asset Library automated in-process WPF acceptance enabled in the isolated Dev Preview runtime.");
            return new(applicationRoot, plan, runRoot!, planPath!, sourceHead!, fixtureRoot);
        }
        catch (Exception exception)
        {
            logService?.Error("P3 Asset Library automated acceptance opt-in was rejected.", exception);
            throw;
        }
    }
    ...
    internal void Observe(MainViewModel viewModel, AssetLibraryP3AutomatedAcceptanceDriver driver)
    {
        ArgumentNullException.ThrowIfNull(viewModel);
    ...
    {
        if (!string.Equals(_scenarioId, FourViewResilienceLayoutScenario, StringComparison.Ordinal))
            throw new InvalidOperationException("Retry command evidence may be recorded only by the resilience scenario.");
    ...
    {
        if (IsRestartPhase)
            throw new InvalidOperationException("A restart scenario may not record fixture bootstrap evidence.");
        var import = new
        {
            mechanism,
    ...
    internal ArtifactState WriteArtifact(string scenarioId, string kind, string fileName, ReadOnlySpan<byte> content)
    {
        ...
        {
            var value when value.Contains("bounds", StringComparison.OrdinalIgnoreCase) => "bounds",
            var value when value.Contains("query-plan", StringComparison.OrdinalIgnoreCase) ||
                           value.Contains("explain", StringComparison.OrdinalIgnoreCase) => "query-plans",
            var value when value.Contains("query-document", StringComparison.OrdinalIgnoreCase) ||
                           value.Contains("canonical", StringComparison.OrdinalIgnoreCase) => "query-documents",
            var value when value.Contains("result-hash", StringComparison.OrdinalIgnoreCase) => "result-hashes",
            var value when value.Contains("history", StringComparison.OrdinalIgnoreCase) => "histories",
            var value when value.Contains("smart-folder", StringComparison.OrdinalIgnoreCase) => "smart-folders",
            var value when value.Contains("membership", StringComparison.OrdinalIgnoreCase) => "memberships",
            var value when value.Contains("tag", StringComparison.OrdinalIgnoreCase) => "tags",
            var value when value.Contains("journal", StringComparison.OrdinalIgnoreCase) => "journals",
            var value when value.Contains("selection", StringComparison.OrdinalIgnoreCase) => "selections",
            var value when value.Contains("view", StringComparison.OrdinalIgnoreCase) => "views",
            var value when value.Contains("command", StringComparison.OrdinalIgnoreCase) ||
                           value.Contains("undo", StringComparison.OrdinalIgnoreCase) ||
                           value.Contains("redo", StringComparison.OrdinalIgnoreCase) => "commands",
            var value when value.Contains("inspector", StringComparison.OrdinalIgnoreCase) => "inspectors",
            var value when value.Contains("performance", StringComparison.OrdinalIgnoreCase) ||
                           value.Contains("timing", StringComparison.OrdinalIgnoreCase) => "performance",
            var value when value.Contains("database", StringComparison.OrdinalIgnoreCase) ||
                           value.Contains("sqlite", StringComparison.OrdinalIgnoreCase) => "databases",
            var value when value.Contains("screenshot", StringComparison.OrdinalIgnoreCase) ||
                           value.EndsWith("png", StringComparison.OrdinalIgnoreCase) => "screenshots",
            _ => throw new InvalidOperationException($"Unknown P3 automated artifact kind '{kind}'."),
        };
        ...
        else if (directoryName == "screenshots") scenario.ScreenshotPaths.Add(artifact.Path);
    ...
    {
        if (attempt != 1 || IsRestartPhase) return;
        cancellationToken.ThrowIfCancellationRequested();
        if (string.IsNullOrWhiteSpace(_fixtureRoot))
            throw new InvalidOperationException("Every P3 primary scenario requires a run-owned fixture.");
        EnsurePathInside(_fixtureDatabasePath, _fixtureRoot, "fixture database");
        if (!File.Exists(_fixtureDatabasePath))
            throw new FileNotFoundException("The selected run-owned P3 fixture database is missing.", _fixtureDatabasePath);
        Directory.CreateDirectory(Path.GetDirectoryName(_databasePath)!);
        if (File.Exists(_databasePath))
            throw new InvalidOperationException("A primary P3 scenario may not reuse an existing runtime database.");
        File.Copy(_fixtureDatabasePath, _databasePath, overwrite: false);
        var counts = string.Equals(_fixtureVariant, "legacy-v6", StringComparison.Ordinal)
            ? new { total_count = 64, active_count = 60, archived_count = 4 }
            : new { total_count = 10128, active_count = 10000, archived_count = 128 };
        RecordEvent(_scenarioId, "run-owned-fixture-copied-before-repository-initialization", "SqliteAssetLibraryRepository", null,
            new { attempt, fixture_variant = _fixtureVariant, fixture_database = _fixtureDatabasePath, fixture_sha256 = HashFile(_fixtureDatabasePath), counts.total_count, counts.active_count, counts.archived_count }, null);
    ...
    {
        if (RestartScenarioIds.Contains(_scenarioId, StringComparer.Ordinal))
        {
        ...
        var summary = new
        {
            ...
            safety_measurement = new
            {
                owner = "independent-runner-after-process-exit",
                status = "pending",
                reason = "process, path, environment, display and sealed-source observations are only complete after this process exits",
            },
            process_cleanup = new
            {
                exit_code = exitCode,
    ...
    private static async Task ValidateDatabaseEvidenceAsync(SqliteConnection connection, ScenarioState state)
    {
        ...
        if (schemaVersion != 7)
            throw new InvalidDataException($"The SQLite evidence backup schema is {schemaVersion}; expected v7 after P3 migration.");
        ...
        await using var fixtureCounts = connection.CreateCommand();
        fixtureCounts.CommandText = "SELECT SUM(CASE WHEN IsArchived=0 THEN 1 ELSE 0 END), SUM(CASE WHEN IsArchived=1 THEN 1 ELSE 0 END) FROM AssetItems;";
        await using var fixtureReader = await fixtureCounts.ExecuteReaderAsync().ConfigureAwait(false);
        if (!await fixtureReader.ReadAsync().ConfigureAwait(false))
            throw new InvalidDataException("The SQLite evidence fixture count query returned no row.");
        var activeCount = fixtureReader.GetInt32(0);
        var archivedCount = fixtureReader.GetInt32(1);
        var legacyMigration = string.Equals(state.Id, SmartFolderInvalidMigrationScenario, StringComparison.Ordinal);
        var expectedTotal = legacyMigration ? 64 : 10128;
        var expectedActive = legacyMigration ? 60 : 10000;
        var expectedArchived = legacyMigration ? 4 : 128;
        if (assetCount != expectedTotal || activeCount != expectedActive || archivedCount != expectedArchived)
            throw new InvalidDataException($"The P3 fixture contract differs (total={assetCount}, active={activeCount}, archived={archivedCount}; expected {expectedTotal}/{expectedActive}/{expectedArchived}).");

        state.Database.SchemaVersion = schemaVersion;
        state.Database.AssetCount = assetCount;
        state.Database.ActiveAssetCount = activeCount;
        state.Database.ArchivedAssetCount = archivedCount;
    }
    ...
    {
        if (!RestartScenarioIds.Contains(_scenarioId, StringComparer.Ordinal))
            throw new InvalidOperationException("Only the three declared P3 persistence scenarios may have a restart phase.");
        if (!File.Exists(_databasePath))
        ...
        string? sourceHead)
    {
        ...
        if (!string.Equals(processName, ExpectedProcessName, StringComparison.Ordinal))
            throw new InvalidOperationException("Automated P3 acceptance is restricted to the Modular Harness Dev Preview executable.");

        foreach (var binaryPath in new[]
                 {
                     Environment.ProcessPath!,
                     typeof(AssetLibraryP3AutomatedAcceptanceController).Assembly.Location,
    ...
    private static AutomatedPlan ReadPlan(string planPath)
    {
        ...
        if (phase == "primary" && (ids.Length != 1 || !FixedScenarioIds.Contains(ids[0], StringComparer.Ordinal)))
            throw new InvalidOperationException("A primary automated process must contain exactly one fixed P3 scenario.");
        if (phase == "restart" &&
            (ids.Length != 1 || !RestartScenarioIds.Contains(ids[0], StringComparer.Ordinal)))
            throw new InvalidOperationException("A restart automated process must contain exactly one declared P3 persistence scenario.");
        var fixtureVariant = RequireString(root, "fixture_variant");
        var fixtureDatabasePath = RequireString(root, "fixture_database_path");
        var expectsLegacyFixture = string.Equals(ids[0], SmartFolderInvalidMigrationScenario, StringComparison.Ordinal);
        var expectedFixtureVariant = expectsLegacyFixture ? "legacy-v6" : "current-v7";
        if (!string.Equals(fixtureVariant, expectedFixtureVariant, StringComparison.Ordinal))
            throw new InvalidOperationException($"Scenario '{ids[0]}' requires fixture_variant '{expectedFixtureVariant}'.");
        if (!Path.IsPathFullyQualified(fixtureDatabasePath) || !File.Exists(fixtureDatabasePath))
            throw new InvalidOperationException("The P3 fixture_database_path must name an existing absolute database file.");
    ...
    private ScenarioState EnsureScenario(string scenarioId) =>
        _scenarios.TryGetValue(scenarioId, out var state)
            ? state
            : throw new InvalidOperationException($"Unknown automated P3 scenario '{scenarioId}'.");
    ...
    private void ValidateLiveBinaryIdentityAtExit()
    {
        ...
        var applicationPath = NormalizePath(typeof(AssetLibraryP3AutomatedAcceptanceController).Assembly.Location);
        ...
        [property: JsonPropertyName("asset_module_sha256")] string AssetModuleSha256)
    {
        ...
        public string HistoricalManualGate => AssetLibraryP3AutomatedAcceptanceController.HistoricalManualGate;
    ...
    private sealed class DatabaseState
    {
        ...
        internal string SchemaQuery { get; set; } = AssetLibraryP3AutomatedAcceptanceController.SchemaQuery;
        internal int? AssetCount { get; set; }
        internal int? ActiveAssetCount { get; set; }
        ...
        internal void Load(JsonElement database)
        {
            ...
            SchemaVersion = database.TryGetProperty("schema_version", out var version) && version.TryGetInt32(out var schemaVersion)
                ? schemaVersion
                : null;
            SchemaQuery = GetString(database, "schema_query") ?? AssetLibraryP3AutomatedAcceptanceController.SchemaQuery;
            AssetCount = database.TryGetProperty("asset_count", out var count) && count.TryGetInt32(out var assetCount)
                ? assetCount
                : null;
            ActiveAssetCount = database.TryGetProperty("active_asset_count", out var active) && active.TryGetInt32(out var activeCount)
                ? activeCount
                : null;
            ArchivedAssetCount = database.TryGetProperty("archived_asset_count", out var archived) && archived.TryGetInt32(out var archivedCount)
                ? archivedCount
                : null;
