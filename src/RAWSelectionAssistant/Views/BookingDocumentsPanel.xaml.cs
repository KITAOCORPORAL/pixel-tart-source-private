...
public partial class BookingDocumentsPanel : UserControl
{
    ...
    private async void BookingDocumentsPanel_Drop(object sender, DragEventArgs e)
    {
        ...
        {
            MessageBox.Show(Window.GetWindow(this), "当前版本只支持添加单个或多个文件，不会扫描文件夹。", "本地摄影资料", MessageBoxButton.OK, MessageBoxImage.Information);
            return;
        }
    ...
    private static void ViewModel_OpenFileRequested(object? sender, string path)
    {
        ...
        catch { MessageBox.Show("文件当前无法打开。", "本地摄影资料", MessageBoxButton.OK, MessageBoxImage.Warning); }
    }

    private static void ViewModel_RevealFileRequested(object? sender, string path)
    {
        try { Process.Start(new ProcessStartInfo("explorer.exe", $"/select,\"{path}\"") { UseShellExecute = true }); }
        catch { MessageBox.Show("文件所在位置当前不可访问。", "本地摄影资料", MessageBoxButton.OK, MessageBoxImage.Warning); }
    }
    ...
    private static void ViewModel_RevealDirectoryRequested(object? sender, string path)
    {
        ...
        catch { MessageBox.Show("输出目录当前不可访问。", "本地摄影资料", MessageBoxButton.OK, MessageBoxImage.Warning); }
    }
