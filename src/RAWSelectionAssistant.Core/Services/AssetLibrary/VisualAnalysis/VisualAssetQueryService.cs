...
public sealed class SqliteVisualAssetQueryService(AssetLibraryDatabase database, IAssetVisualFeatureStore featureStore) : IVisualAssetQueryService, IVisualAssetQueryDiagnosticsProvider
{
    ...
    public VisualSimilarityDiagnostics? LastSimilarityDiagnostics => Volatile.Read(ref _lastSimilarityDiagnostics);
    public event Action<VisualSimilarityDiagnostics>? SimilarityDiagnosticsUpdated;
    ...
    public async Task<IReadOnlyList<VisualSimilarityMatch>> FindSimilarAsync(VisualSimilarityQuery query, CancellationToken cancellationToken = default)
    {
        ...
        {
            PublishSimilarityDiagnostics(new(0, totalStopwatch.Elapsed.TotalMilliseconds, 0, totalStopwatch.Elapsed.TotalMilliseconds, 0));
            return [];
        }
        ...
        if (query.Mode == VisualSimilarityMode.Palette)
        {
            ...
            var paletteResults = paletteCandidates
                ...
                .ToArray();
            paletteScoringStopwatch.Stop(); totalStopwatch.Stop();
            PublishSimilarityDiagnostics(new(paletteCandidates.Count, palettePruningStopwatch.Elapsed.TotalMilliseconds, paletteScoringStopwatch.Elapsed.TotalMilliseconds, totalStopwatch.Elapsed.TotalMilliseconds, paletteResults.Length));
        ...
        var results = candidates
            ...
            .Take(query.EffectiveLimit)
            .ToArray();
        scoringStopwatch.Stop(); totalStopwatch.Stop();
        PublishSimilarityDiagnostics(new(candidates.Count, pruningStopwatch.Elapsed.TotalMilliseconds, scoringStopwatch.Elapsed.TotalMilliseconds, totalStopwatch.Elapsed.TotalMilliseconds, results.Length));
        return results;
    }

    private void PublishSimilarityDiagnostics(VisualSimilarityDiagnostics diagnostics)
    {
        Volatile.Write(ref _lastSimilarityDiagnostics, diagnostics);
        SimilarityDiagnosticsUpdated?.Invoke(diagnostics);
    }
    ...
    private static string BuildWhere(AssetLibraryQuery scope, VisualAssetFilter filter, SqliteCommand command, bool usesResolvedScope)
    {
        ...
        if (filter.DominantHue is { } hue)
        {
            ...
            where += " AND a.AssetId IN (SELECT pc.AssetId FROM AssetVisualPaletteColors pc WHERE pc.AnalysisVersion=$visualVersion AND pc.Weight>=0.15 AND pc.Saturation>=0.08 AND pc.Chroma>=8 AND " + condition + ")";
        }
    ...
    private static VisualAssetMatch ReadMatch(SqliteDataReader reader)
    {
        ...
        var summary = new AssetVisualFeatureSummary
        {
            ...
            State = state,
            AnalysisVersion = reader.GetString(20),
            ContentFingerprint = reader.GetString(21),
            SourceContentHash = reader.IsDBNull(22) ? null : reader.GetString(22),
            FailureReason = reader.IsDBNull(24) ? null : reader.GetString(24),
            AnalysisSource = EnumValue<VisualAnalysisSourceKind>(25) ?? VisualAnalysisSourceKind.RasterOriginal,
            SourceProfile = reader.GetString(26),
            AnalysisProfile = reader.GetString(27),
            Harmony = EnumValue<ColorHarmonyTendency>(28),
            ToneKey = EnumValue<ToneKeyTendency>(29),
            Contrast = EnumValue<ContrastTendency>(30),
            LuminanceSpan = EnumValue<LuminanceSpanTendency>(31),
            Saturation = EnumValue<SaturationTendency>(32),
            WarmCool = EnumValue<WarmCoolTendency>(33),
            DominantHue = Number(34),
            SecondaryHue = Number(35),
            AverageHue = Number(36),
            AverageLuma = Number(37),
            MedianLuma = Number(38),
            ContrastMetric = Number(39),
            LumaSpreadMetric = Number(40),
            AverageSaturation = Number(41),
            MedianSaturation = Number(42),
            AverageLightness = Number(43),
            WarmCoolMetric = Number(44),
            DeepShadowRatio = Number(45),
            ShadowRatio = Number(46),
            MidtoneRatio = Number(47),
            HighlightRatio = Number(48),
            SpecularRatio = Number(49),
            BlackClipRatio = Number(50),
            WhiteClipRatio = Number(51),
            HistogramLumaSignature = reader.IsDBNull(52) ? null : reader.GetString(52),
            PaletteSignature = reader.IsDBNull(53) ? null : reader.GetString(53),
            CreatedAt = DateTimeOffset.Parse(reader.GetString(54)),
            UpdatedAt = DateTimeOffset.Parse(reader.GetString(55))
        };
