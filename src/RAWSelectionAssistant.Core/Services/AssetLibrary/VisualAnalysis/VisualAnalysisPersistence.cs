...
public sealed class SqliteAssetVisualAnalysisCache(AssetLibraryDatabase database) : IAssetVisualFeatureStore
{
    ...
    internal static AssetVisualFeatureSummary ReadSummary(SqliteDataReader reader, Guid assetId, AssetVisualFeatureState state)
    {
        ...
        return new()
        {
            AssetId = assetId,
            State = state,
            AnalysisVersion = reader.GetString(0),
            ContentFingerprint = reader.GetString(1),
            SourceContentHash = reader.IsDBNull(2) ? null : reader.GetString(2),
            AnalysisSource = Parse<VisualAnalysisSourceKind>(reader, 5) ?? VisualAnalysisSourceKind.RasterOriginal,
            SourceProfile = reader.GetString(6),
            AnalysisProfile = reader.GetString(7),
            Harmony = Parse<ColorHarmonyTendency>(reader, 8),
            ToneKey = Parse<ToneKeyTendency>(reader, 9),
            Contrast = Parse<ContrastTendency>(reader, 10),
            LuminanceSpan = Parse<LuminanceSpanTendency>(reader, 11),
            Saturation = Parse<SaturationTendency>(reader, 12),
            WarmCool = Parse<WarmCoolTendency>(reader, 13),
            DominantHue = Number(14),
            SecondaryHue = Number(15),
            AverageHue = Number(16),
            AverageLuma = Number(17),
            MedianLuma = Number(18),
            ContrastMetric = Number(19),
            LumaSpreadMetric = Number(20),
            AverageSaturation = Number(21),
            MedianSaturation = Number(22),
            AverageLightness = Number(23),
            WarmCoolMetric = Number(24),
            DeepShadowRatio = Number(25),
            ShadowRatio = Number(26),
            MidtoneRatio = Number(27),
            HighlightRatio = Number(28),
            SpecularRatio = Number(29),
            BlackClipRatio = Number(30),
            WhiteClipRatio = Number(31),
            HistogramLumaSignature = reader.IsDBNull(32) ? null : reader.GetString(32),
            PaletteSignature = reader.IsDBNull(33) ? null : reader.GetString(33),
            CreatedAt = DateTimeOffset.Parse(reader.GetString(35)),
            UpdatedAt = DateTimeOffset.Parse(reader.GetString(36)),
