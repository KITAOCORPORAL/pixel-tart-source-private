...
public sealed partial class SqliteAssetLibraryRepository : IAssetLibraryRepository
{
    ...
    {
        if (memberships.Count == 0) return; await using var connection = await _database.OpenConnectionAsync(write: true, cancellationToken).ConfigureAwait(false); await using var transaction = (SqliteTransaction)await connection.BeginTransactionAsync(cancellationToken).ConfigureAwait(false); foreach (var item in memberships) { await using var command = connection.CreateCommand(); command.Transaction = transaction; command.CommandText = "INSERT OR IGNORE INTO AssetFolderMemberships(AssetId,FolderId,AddedAt) VALUES($asset,$folder,$at);"; command.Parameters.AddWithValue("$asset", item.AssetId.ToString("D")); command.Parameters.AddWithValue("$folder", item.FolderId.ToString("D")); command.Parameters.AddWithValue("$at", item.AddedAt.ToString("O")); await command.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false); }
        await transaction.CommitAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        if (memberships.Count == 0) return; await using var connection = await _database.OpenConnectionAsync(write: true, cancellationToken).ConfigureAwait(false); await using var transaction = (SqliteTransaction)await connection.BeginTransactionAsync(cancellationToken).ConfigureAwait(false); foreach (var item in memberships) { await using var command = connection.CreateCommand(); command.Transaction = transaction; command.CommandText = "INSERT OR IGNORE INTO AssetTagMemberships(AssetId,TagId,AddedAt) VALUES($asset,$tag,$at);"; command.Parameters.AddWithValue("$asset", item.AssetId.ToString("D")); command.Parameters.AddWithValue("$tag", item.TagId.ToString("D")); command.Parameters.AddWithValue("$at", item.AddedAt.ToString("O")); await command.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false); }
        await transaction.CommitAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        await using var connection = await _database.OpenConnectionAsync(write: true, cancellationToken).ConfigureAwait(false); await using var transaction = (SqliteTransaction)await connection.BeginTransactionAsync(cancellationToken).ConfigureAwait(false); foreach (var asset in assetIds) { await using var command = connection.CreateCommand(); command.Transaction = transaction; command.Parameters.AddWithValue("$asset", asset.ToString("D")); command.Parameters.AddWithValue("$folder", folderId.ToString("D")); if (add) { command.CommandText = "INSERT OR IGNORE INTO AssetFolderMemberships(AssetId,FolderId,AddedAt) VALUES($asset,$folder,$at);"; command.Parameters.AddWithValue("$at", DateTimeOffset.UtcNow.ToString("O")); } else command.CommandText = "DELETE FROM AssetFolderMemberships WHERE AssetId=$asset AND FolderId=$folder;"; await command.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false); }
        await transaction.CommitAsync(cancellationToken).ConfigureAwait(false);
    }
