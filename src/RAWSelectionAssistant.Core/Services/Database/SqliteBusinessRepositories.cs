...
public sealed class SqliteFinanceRepository(IPixelTartDatabase database) : IFinanceRepository
{
    ...
    public async Task<IReadOnlyList<FinanceTransaction>> QueryAsync(FinanceQuery query, CancellationToken cancellationToken = default)
    {
        ...
        if (query.From is not null) { clauses.Add("OccurredOn >= $from"); command.Parameters.AddWithValue("$from", query.From.Value.ToString("yyyy-MM-dd")); }
        if (query.To is not null) { clauses.Add("OccurredOn <= $to"); command.Parameters.AddWithValue("$to", query.To.Value.ToString("yyyy-MM-dd")); }
        if (query.Kind is not null) { clauses.Add("Kind=$kind"); command.Parameters.AddWithValue("$kind", query.Kind.Value.ToString()); }
        if (query.PaymentStatus is not null) { clauses.Add("PaymentStatus=$status"); command.Parameters.AddWithValue("$status", query.PaymentStatus.Value.ToString()); }
        if (query.BookingId is not null) { clauses.Add("BookingId=$booking"); command.Parameters.AddWithValue("$booking", query.BookingId.Value.ToString("D")); }
        if (query.ProjectId is not null) { clauses.Add("ProjectId=$project"); command.Parameters.AddWithValue("$project", query.ProjectId.Value.ToString("D")); }
        if (query.CategoryId is not null) { clauses.Add("CategoryId=$category"); command.Parameters.AddWithValue("$category", query.CategoryId.Value.ToString("D")); }
        if (!string.IsNullOrWhiteSpace(query.CurrencyCode)) { clauses.Add("CurrencyCode=$currency COLLATE NOCASE"); command.Parameters.AddWithValue("$currency", query.CurrencyCode.Trim()); }
        if (!string.IsNullOrWhiteSpace(query.Keyword)) { clauses.Add("(Counterparty LIKE $keyword OR PaymentMethod LIKE $keyword OR Note LIKE $keyword OR EXISTS(SELECT 1 FROM ShootBookings b WHERE b.Id=FinanceTransactions.BookingId AND (b.Title LIKE $keyword OR b.ClientDisplayName LIKE $keyword)) OR EXISTS(SELECT 1 FROM Projects p WHERE p.Id=FinanceTransactions.ProjectId AND p.Name LIKE $keyword))"); command.Parameters.AddWithValue("$keyword", "%" + query.Keyword.Trim() + "%"); }
