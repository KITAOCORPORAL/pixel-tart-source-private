...
public sealed class SidebarInteractionFix202Tests
{
    ...
    [TestMethod] public void Requirement44_AllPrimaryEntries_UseStableTemplate() { var sidebar = SidebarSlice(); foreach (var name in new[] { "工作台", "归片工作区", "工作日历", "项目历史", "授权与版本", "设置", "帮助" }) Contains(sidebar, $"Content=\"{name}\""); DoesNotContain(sidebar, "Content=\"本地分片\""); Assert.IsGreaterThanOrEqualTo(11, Count(sidebar, "SidebarNavButton")); }
    [TestMethod] public void Requirement45_ToolboxIsGroupedAndFooterKeepsEditionAndCollapse() { Contains(SidebarSlice(), "Text=\"工具\"", "ToolboxEntry.DisplayName", "Grid.Row=\"3\"", "Grid.Row=\"4\"", "IconPanelExpand", "展开侧栏"); DoesNotContain(SidebarSlice(), "Content=\"使用教程\"", "Content=\"问题反馈\""); Contains(MainXaml(), "Content=\"打开帮助与教程\"", "Content=\"建议与问题反馈\""); }
    [TestMethod] public void Requirement46_EditionStatus_HidesLongTextWhenCollapsed() => Contains(SidebarSlice(), "x:Name=\"EditionStatusArea\"", "授权与版本", "IsSidebarExpanded", "IsSidebarCollapsed");
    [TestMethod] public void Requirement47_AllEntries_HaveTooltipsAndAutomationNames() { var sidebar = SidebarSlice(); Assert.IsGreaterThanOrEqualTo(13, Count(sidebar, "ToolTip=")); Assert.IsGreaterThanOrEqualTo(13, Count(sidebar, "AutomationProperties.Name=")); }
