...
public sealed class AssetLibraryP2CoreTests
{
    ...
    public async Task AssetFlagsAndFolderRestoreAreMetadataOnlyUndoableAndPersistentlyRedoable()
    {
        ...
        Assert.AreEqual(8L, (long)(await schema.ExecuteScalarAsync())!, "AL-F01 keeps the main application schema unchanged while intentionally upgrading only the private Asset Library schema to v8.");
