...
function Request-NativeEvidence([string]$folder,[string]$label) {
  $nonce=[guid]::NewGuid().ToString();$request=Join-Path $folder 'native-observe-request.txt';$response=Join-Path $folder 'native-observe-response.json';$tmp="$request.$nonce.tmp";Set-Content $tmp $nonce -Encoding ASCII;Move-Item -Force $tmp $request;$deadline=(Get-Date).AddSeconds(3)
...
function Native-Click([int]$x,[int]$y){Native-Move $x $y;[PixelTartNativeInput]::Mouse([PixelTartNativeInput]::MOUSEEVENTF_LEFTDOWN)|Out-Null;Start-Sleep -Milliseconds 60;[PixelTartNativeInput]::Mouse([PixelTartNativeInput]::MOUSEEVENTF_LEFTUP)|Out-Null;Start-Sleep -Milliseconds 350}
function Native-Key([uint16]$key){[PixelTartNativeInput]::Key($key)|Out-Null;Start-Sleep -Milliseconds 60;[PixelTartNativeInput]::Key($key,$true)|Out-Null;Start-Sleep -Milliseconds 400}
function Native-Modified([uint16]$modifier,[uint16]$key){[PixelTartNativeInput]::Key($modifier)|Out-Null;Start-Sleep -Milliseconds 60;Native-Key $key;[PixelTartNativeInput]::Key($modifier,$true)|Out-Null;Start-Sleep -Milliseconds 500}
function Drag-Visual([int]$sourceIndex,[int]$destinationIndex,[bool]$after,[string]$id) {
  $rows=@(Get-NodeRows);if($rows.Count -lt 4){throw "Expected four node rows; found $($rows.Count)."};$source=$rows[$sourceIndex];$dest=$rows[$destinationIndex];$sr=Get-Rect $source;$dr=Get-Rect $dest;$sx=[int]($sr.X+$sr.W/2);$sy=[int]($sr.Y+$sr.H/2);$tx=[int]($dr.X+$dr.W/2);$fraction=if($after){.78}else{.22};$ty=[int]($dr.Y+($dr.H*$fraction));$case=[ordered]@{Id=$id;BeforeOrder=@(Get-NodeNames);Stages=[ordered]@{}}
  $case.Stages.Before=[ordered]@{Screenshot=(Capture-One $script:hwnd $selected ("drag_{0}_01_BEFORE"-f$id));Evidence=(Request-NativeEvidence (Join-Path $env:PIXEL_TART_ISOLATED_RUNTIME_ROOT 'ColorStudioFixture') "$id-before")}
  Native-Move $sx $sy;[PixelTartNativeInput]::Mouse([PixelTartNativeInput]::MOUSEEVENTF_LEFTDOWN)|Out-Null;Start-Sleep -Milliseconds 120;[PixelTartNativeInput]::Move($sx+2,$sy+2)|Out-Null;Start-Sleep -Milliseconds 180
  $case.Stages.Threshold=[ordered]@{Screenshot=(Capture-One $script:hwnd $selected ("drag_{0}_02_THRESHOLD"-f$id));Evidence=(Request-NativeEvidence (Join-Path $env:PIXEL_TART_ISOLATED_RUNTIME_ROOT 'ColorStudioFixture') "$id-threshold")}
  [PixelTartNativeInput]::Move($tx,$ty)|Out-Null;Start-Sleep -Milliseconds 260
  $case.Stages.Insertion=[ordered]@{Screenshot=(Capture-One $script:hwnd $selected ("drag_{0}_03_INSERTION"-f$id));Evidence=(Request-NativeEvidence (Join-Path $env:PIXEL_TART_ISOLATED_RUNTIME_ROOT 'ColorStudioFixture') "$id-insertion")}
  [PixelTartNativeInput]::Mouse([PixelTartNativeInput]::MOUSEEVENTF_LEFTUP)|Out-Null;Start-Sleep -Milliseconds 650
  $case.Stages.Drop=[ordered]@{Screenshot=(Capture-One $script:hwnd $selected ("drag_{0}_04_DROP"-f$id));Evidence=(Request-NativeEvidence (Join-Path $env:PIXEL_TART_ISOLATED_RUNTIME_ROOT 'ColorStudioFixture') "$id-drop")}
  $case.Stages.Final=[ordered]@{Screenshot=(Capture-One $script:hwnd $selected ("drag_{0}_05_FINAL"-f$id));Evidence=(Request-NativeEvidence (Join-Path $env:PIXEL_TART_ISOLATED_RUNTIME_ROOT 'ColorStudioFixture') "$id-final")};$case.AfterOrder=@(Get-NodeNames);return [pscustomobject]$case
}
Add-Type @'
...
$dragCases=@();$cancelCase=$null;if($RunDragMatrix -and $selected){$specs=@(@{Id='N1-N2';S=0;D=1;A=$true},@{Id='N1-N3';S=0;D=2;A=$false},@{Id='LAST-FIRST';S=3;D=0;A=$false},@{Id='MIDDLE-LAST';S=1;D=3;A=$true},@{Id='ADJACENT';S=1;D=2;A=$true},@{Id='NOOP';S=1;D=1;A=$false});foreach($spec in $specs){for($rep=1;$rep -le $DragRepetitions;$rep++){$case=Drag-Visual $spec.S $spec.D $spec.A ("{0}-{1}"-f$spec.Id,$rep);$dragCases+=$case;Native-Modified 0x11 0x5A}}}
$manifest=[ordered]@{RunId=$runId;ProductSourceSha=(git rev-parse HEAD).Trim();WindowDiagnostics=$dr;CaptureDiagnosis=$diagnosticCaptures;SelectedCaptureMethod=$selected;Stability=$counts;StabilityTarget=$StabilityCount;NativeProductionWindow=($null -ne $selected);DragMatrix=$dragCases;EscCancel=$cancelCase;StartedAtUtc=$started.ToString('o');CompletedAtUtc=[DateTime]::UtcNow.ToString('o')};$manifest|ConvertTo-Json -Depth 25|Set-Content (Join-Path $runRoot 'COLOR_STUDIO_PHASE1_NATIVE_CAPTURE_MANIFEST.json') -Encoding UTF8
