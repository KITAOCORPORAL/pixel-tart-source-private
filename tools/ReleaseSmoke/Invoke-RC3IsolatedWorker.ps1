...
$process=$null
function Get-Root([int]$processId,[int]$timeout=25){$condition=New-Object System.Windows.Automation.PropertyCondition([System.Windows.Automation.AutomationElement]::ProcessIdProperty,$processId);$end=[DateTime]::UtcNow.AddSeconds($timeout);do{foreach($window in @([System.Windows.Automation.AutomationElement]::RootElement.FindAll([System.Windows.Automation.TreeScope]::Children,$condition))){try{if($window.Current.ControlType-eq[System.Windows.Automation.ControlType]::Window-and-not$window.Current.IsOffscreen){return $window}}catch{}};Start-Sleep -Milliseconds 250}while([DateTime]::UtcNow-lt$end);$null}
